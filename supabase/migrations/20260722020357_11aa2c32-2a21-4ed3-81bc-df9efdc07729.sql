...
AS $function$
DECLARE
  q text := lower(extensions.unaccent(trim(coalesce(search_query, ''))));
  tokens text[];
BEGIN
  IF length(q) < 2 THEN
    RETURN;
  ...
  WITH base AS (
    SELECT
      ...
      lower(extensions.unaccent(s.name)) AS lname
  ...
  scored AS (
    SELECT
      b.*,
      extensions.similarity(b.lname, q) AS sim,
    ...
    WHERE
      b.lname LIKE '%' || q || '%'
      OR extensions.similarity(b.lname, q) > 0.25
      OR (
        SELECT bool_and(b.lname LIKE '%' || tok || '%')
        FROM unnest(tokens) AS tok
        WHERE length(tok) > 0
