          ...
          {filters.map((f, i) => (
            <button key={f.key} onClick={() => setActiveFilter(f.key)}
              ...
              className={cn("px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-all",
                activeFilter === f.key ? "bg-primary text-primary-foreground" : "bg-secondary/50 text-muted-foreground hover:bg-secondary")}>
              {f.label}
            </button>
          ))}
        </div>
        <main className="relative z-10 px-4 py-4 space-y-3 animate-fade-in">
      ...
      <div className="flex h-[calc(100vh-4rem)]">
        <div className="w-[340px] flex-shrink-0 border-r border-border/50 flex flex-col bg-background/60 animate-fade-in">
          <div className="px-4 py-3 border-b border-border/50">
            ...
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <GradientInput placeholder="Search..." className="pl-9 h-8 text-sm"
                value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
          </div>
