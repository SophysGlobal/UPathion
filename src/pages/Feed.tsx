    ...
    <div className="gradient-border">
      <div className="bg-card/90 backdrop-blur-sm rounded-lg p-4">
        <div className="flex items-center gap-3 mb-3">
          <button onClick={() => onUserClick(post.authorName)}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <button onClick={() => onUserClick(post.authorName)} className="font-medium text-foreground hover:underline">{post.authorName}</button>
              {post.authorBadge && <span className="px-2 py-0.5 rounded-full bg-accent/20 text-accent text-xs font-medium">{post.authorBadge}</span>}
            </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {post.tags.map((tag) => (
            <span key={tag} className="text-xs text-primary font-medium hover:underline cursor-pointer">{tag}</span>
          ))}
        </div>
        <div className="flex items-center gap-6 pt-3 border-t border-border/50">
          <button onClick={() => setLiked(!liked)} className="flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors">
...
const Feed = () => {
  ...
  const handleSchoolClick = (schoolName: string) => { setSelectedSchool(schoolName); setSchoolSheetOpen(true); };
  const handleUserClick = (authorName: string) => {
    const post = posts.find(p => p.authorName === authorName);
    setSelectedUser({ name: authorName, role: post?.authorRole, badge: post?.authorBadge, school: post?.schoolName });
  ...
  const renderEmptyState = () => (
    <div className="text-center py-12 col-span-full">
      ...
      <p className="text-sm text-muted-foreground max-w-xs mx-auto">Posts from your school community will appear here</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-background/80 pb-20 relative">
          ...
          {filters.filter(f => f.show).map((filter) => (
            <button key={filter.key} onClick={() => setActiveFilter(filter.key)}
              className={`px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-all ${
                activeFilter === filter.key ? 'bg-primary text-primary-foreground' : 'bg-secondary/50 text-muted-foreground hover:bg-secondary'
              }`}>{filter.label}</button>
          ))}
        </div>
      </div>
      ...
      <SchoolBottomSheet open={schoolSheetOpen} onOpenChange={setSchoolSheetOpen}
        school={selectedSchool ? { name: selectedSchool, type: selectedSchool.toLowerCase().includes('high') ? 'high_school' : 'university' } : null}
